...
public static class ServiceDetector
{
    ...
    {
        DetectedServices? result = null;
        
        await AnsiConsole.Status()
            .Spinner(Spinner.Known.Dots)
            .SpinnerStyle(Style.Parse("cyan"))
            .StartAsync("Detecting services...", async _ =>
            {
                result = await DetectAsync(config, verbose);
            });
        ...
        else if (!result.DoclingHasGpu && !forceGpu.HasValue)
        {
            ...
            if (verbose)
                tips.Add("[dim]Docling running on CPU[/] - use --docling-gpu if you have CUDA");
        }
        ...
        if (verbose)
        {
            ...
            {
                AnsiConsole.MarkupLine($"[dim]  Docling accelerator: {result.DoclingAccelerator ?? "CUDA"}[/]");
            }
