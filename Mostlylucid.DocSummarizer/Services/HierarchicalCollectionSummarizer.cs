...
public class HierarchicalCollectionSummarizer : IAsyncDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (_verbose)
        {
            AnsiConsole.MarkupLine($"[cyan]Collection Analysis:[/] {Markup.Escape(collectionInfo.ToString())}");
        }
        ...
        if (_verbose)
        {
            AnsiConsole.MarkupLine($"[dim]Partitioned into {works.Count} works[/]");
        }
        ...
        {
            AnsiConsole.MarkupLine($"[dim]Sampled {selectedWorks.Count} representative works from {works.Count}[/]");
        }
        ...
        CancellationToken ct)
    {
        ...
        await AnsiConsole.Progress()
            ...
            {
                var task = ctx.AddTask($"[cyan]Summarizing {works.Count} works[/]", maxValue: works.Count);

                foreach (var work in works)
                {
                    ct.ThrowIfCancellationRequested();
                    
                    task.Description = $"[cyan]Summarizing: {Truncate(work.Title, 40)}[/]";
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (_verbose)
                        {
                            AnsiConsole.MarkupLine($"[red]Failed to summarize '{Markup.Escape(work.Title)}': {Markup.Escape(ex.Message)}[/]");
                        }
