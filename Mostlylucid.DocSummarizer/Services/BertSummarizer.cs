...
public class BertSummarizer : IDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (_verbose) AnsiConsole.MarkupLine("[dim]Parsing document with Markdig...[/]");
        ...
        if (_verbose) AnsiConsole.MarkupLine($"[dim]Found {parsedDoc.SentenceCount} sentences in {parsedDoc.Sections.Count} sections[/]");
        ...
        if (_verbose) AnsiConsole.MarkupLine("[dim]Generating BERT embeddings...[/]");
        ...
        if (_verbose) AnsiConsole.MarkupLine($"[dim]Selecting {targetCount} sentences using MMR...[/]");
    ...
    private async Task GenerateEmbeddingsAsync(List<SentenceInfo> sentences, CancellationToken ct)
    {
        ...
        for (int i = 0; i < total; i += batchSize)
        {
            ...
            if (_verbose && total > batchSize)
            {
                ...
                AnsiConsole.MarkupLine($"[dim]  Embedded {progress}/{total} sentences[/]");
            }
        }
    }
