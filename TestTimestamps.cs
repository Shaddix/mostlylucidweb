...
Console.WriteLine($"\nA date in 2025-01-15: {typical:O}");
Console.WriteLine($"Its timestamp: {typicalMs}");

// Current timestamp range (approx Nov 2024)
var currentDate = new DateTime(2024, 11, 19, 0, 0, 0, DateTimeKind.Utc);
var currentOffset = new DateTimeOffset(currentDate);
var currentMs = currentOffset.ToUnixTimeMilliseconds();
Console.WriteLine($"\nNov 19, 2024: {currentDate:O}");
Console.WriteLine($"Its timestamp: {currentMs}");
