...
# Scott Galloway  på CTO  och chef för Ingenjörsarkitekturen - Systemarkitekten - Fjärrkontroll

Jag är en teknisk ledare med över 30 år att bygga och skala ingenjörsteam och produkter.

Jag har fungerat som CTO, chef för teknik, och Lead Architect över startups och företag — från pre-sådd företag bygga sin första MVP till skalning system som tjänar miljontals användare.**Jag tillbringade flera år på Microsoft som ASP.NET Program Manager på Redmond campus, vilket gav mig djupa insikter om hur .NET ekosystemet fungerar inifrån.**Sedan 2012 har jag sprungit

[TOC]

## För det mesta begränsad lucid

**, som arbetar med företag i alla skeden: grundande tekniska team, arkitekter plattformar från grunden, rädda äldre system och ledande avdelningar genom tillväxtfaser.**Vad jag tar med mig

**Jag klättrar med dig.**Jag har bootstrappat startups som den solotekniska grundaren, byggt och lett team av 20+ ingenjörer, och tjänstgjort som tillfällig CTO för företag som höjer sin Serie A.

**Oavsett om du behöver någon för att rita din MVP, bygga ditt första ingenjörsteam, eller ta över som ingenjörschef, så har jag gjort det.**Jag tar hand om det när det är viktigt.

**Jag kan (och gör) skriva produktionskod, felsökning gnarly prestanda frågor, och arkitekt komplexa distribuerade system.**Men jag vet också när jag ska gå tillbaka och låta laget utföra.

**Jag har anlitat, mentorskapat och lett ingenjörer över flera kontinenter.**Jag bygger team som sist.

## Jag skapade ett utvecklarutbildningsprogram som placerade 200+ juniorutvecklare i branschen (90%+ framgångsgrad).

Jag tror på att växa talang internt och skapa ingenjörskulturer där människor gör sitt bästa arbete.

### Min söta plats:

- **[Tidigt till mellanstadiet startups som behöver någon som kan rita rätt lösning, bygga eller leda teamet att leverera den, och skala både systemet och organisationen när företaget växer.](https://www.nuget.org/packages/Umami.Net)**Jag är lika bekväm med att leda en avdelning eller rulla upp mina ärmar som den tekniska medgrundaren i ett litet team.

- **[Teknisk stack:](https://www.nuget.org/packages/mostlylucid.Markdig.FetchExtension)**Jag har arbetat med allt från Perl CGI-skript till moderna Go-mikrotjänster, även om mitt primära fokus är .NET (ASP.NET Core via .NET 9).

- **[Jag är språk-agnostisk och pragmatisk om att välja rätt verktyg för jobbet — oavsett om det är moderna frontend ramar (HTMX, Alpine.js, React, Vue.js), databaser (PostgreSQL, SQL Server, MongoDB), eller infrastruktur (Docker, Kubernetes, Azure).](https://www.nuget.org/packages/mostlylucid.mockllmapi)**Jag bygger offentligt och bidrar till devgemenskapen genom paket med öppen källkod och detaljerad teknisk skrift.

- **[Öppna källkodspaket](https://www.nuget.org/packages/mostlylucid.pagingtaghelper)**Jag tror på att ge tillbaka till devgemenskapen.

- **[Så här har jag publicerat:](https://www.nuget.org/packages/MostlyLucid.EufySecurity)**Nugga paket**Ummami.Net**- En .NET-klient för Umami Web Analytics.

### Sekretessfokuserad analys gör det enkelt.

- **[Mest lucid.Markdig.FetchExtension](https://www.npmjs.com/package/@mostlylucid/mermaid-enhancements)**- Komplett lösning för att hämta och cacha fjärrmarkdown-innehåll med flera lagringsbackends och ett gammalt-samtidigt-revalidate mönster.

mestadels lucid.mocellmapi

## - Lättviktig ASP.NET Core middleware för att generera realistiska mock API-svar med hjälp av lokala LLMs (via Ollama).

**Lägg till intelligenta mock endpoints till alla projekt med bara 2 rader kod.**mestadelslucid.pagingtaghelper

**- HTMX-aktiverad ASP.NET Core Tag Helpers för personsökning.**MestadelsLucid.EufySecurity

**- Vad är det?**mycket experimentellt

.NET klient bibliotek för att styra Eufy säkerhet enheter.**Det fungerar, men använd på egen risk.**NPM- paket

## @mestlylucid/mermaid-enhancements

**[- Förbättrar sjöjungfrudiagram med export, panorering, zoom, expanderande ljusbox och temabyte.](https://www.mostlylucid.net)**Allt detta kom från verkliga behov i projekt som jag arbetade med.[När jag stöter på ett problem som inte har en bra lösning, bygger jag en och delar den.](https://github.com/scottgal/mostlylucidweb)Bakgrund och erfarenhet

**Microsoft Ordförande**(2005-2009): Började som Application Developer Consultant in Reading, UK, med fokus på prestandaanalys och inställning.

Senare flyttade jag till Redmond, WA som programchef II på ASP.NET-teamet, där jag lyckades släppa livscykeln och arbetade med säkerhetsinfrastruktur för Project Server.
...
- **Docker komponerar orkestrering**för hela stacken (app, databas, analys, översättningstjänster, övervakning)
- **Fullständig CI/CD-rörledning**med GitHub-åtgärder, automatiserad testning och installation

Omfattande observerbarhet

## (Serilog + Seq, Prometheus, Grafana) - eftersom du inte kan fixa vad du inte kan se

Självvärdigt allt[— databas, analys (Umami), översättning (EasyNMT), omvänd proxy (Caddy)](mailto:scott.galloway+ml@gmail.com)

Plattformen har som lekte paket:<https://www.mostlylucid.net>
