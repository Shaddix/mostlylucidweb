...
So you might have noticed hundreds of ["new" blog posts](https://www.mostlylucid.net/blog/category/Imported) appearing recently. Well, they're not new at all - they're OLD. Like, 2004 old. I finally built a tool to rescue my content from the digital graveyard that was my old blog at mostlylucid.co.uk.

## Why Archive.org is Absolutely Brilliant

Before I dive into the technical stuff, I have to give a massive shoutout to the [Internet Archive](https://archive.org/) and their Wayback Machine. This non-profit organisation has been quietly archiving the web since 1996, preserving billions of web pages that would otherwise be lost forever.
...
From memory it used Some custom thing then [SubText](http://beletsky.net/2010/09/subtext-open-source-blogging-engine.html) (Phil Haack's blogging thing). Followed by Community Server (a [Telligent](https://community.telligent.com/) thing ASP.NET used to use to host sites, by ANOTHER ex ASP,NET PM Rob Howard) . ALL of which had different ways of updating and different ways of displaying content.
...
- `<!-- BEGIN WAYBACK TOOLBAR INSERT -->...<!-- END WAYBACK TOOLBAR INSERT -->` - the toolbar HTML
- `<script>` tags referencing `playback.archive.org`
- Archive metadata comments
- URL prefixes like `https://web.archive.org/web/20040527/` that get prepended to all links
...
The `ExtractMainContent` method uses a hierarchical search to find the content:
...
The final output includes my blog's front matter format:
...
Now for the fun part. Old blog posts often had no tags at all, or tags that didn't make sense for my current site structure. So I integrated Ollama to analyse content and generate relevant tags.
...
I use `gemma3:4b` as it's fast and small enough to run locally without much fuss. The low temperature (0.3) keeps the output consistent – we don't want creative hallucinations in our tags. Note; for my blog this worked as the posts were short, if yours is longer be sure to look at chunking and summarisation techniques fo fit in your model's context window.
...
The response parsing is defensive – if Ollama returns rubbish or times out, we just get an empty tag list rather than crashing the entire pipeline.
...
1. **It's very specific to MY blog** – The selectors, patterns, and date extraction are all tuned for mostlylucid.co.uk. You'd need to customise everything for a different site.
...
If you want to adapt this for your own site (it'll need customisation), the commands are:
...
After running this on my old blog, I recovered posts dating back to [January 1st, 2004](/blog/365) and before! Reading through them is a fascinating trip through time. Web development discussions from before jQuery existed. Posts about technologies that are now completely obsolete. And some embarrassing opinions I held as a younger developer.

You can find all the imported posts using the [Imported](/blog/category/Imported) category tag.

## What's Next

The imported content has a LOT of dead links – that's just the nature of 20-year-old web content. I [built a middleware solution](/blog/the-war-on-404) that:
