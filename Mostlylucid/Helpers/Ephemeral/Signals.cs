    ...
    SignalPropagation? Propagation = null)
{
    ...
    /// </summary>
    public bool WouldCycle(ReadOnlySpan<char> signal) => Propagation?.Contains(signal) == true;
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool Is(ReadOnlySpan<char> name) => Signal.AsSpan().SequenceEqual(name);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool StartsWith(ReadOnlySpan<char> prefix) => Signal.AsSpan().StartsWith(prefix);
}
...
public sealed class SignalPropagation
{
    ...
    private SignalPropagation(string[] path) => _path = path;

    /// <summary>
    /// Create the root of a propagation chain.
    /// </summary>
    public static SignalPropagation Root(string signal) => new([signal]);
    ...
    public bool Contains(ReadOnlySpan<char> signal)
    {
        foreach (var s in _path)
        {
            if (s.AsSpan().SequenceEqual(signal)) return true;
        }
        return false;
    }
...
public sealed class SignalConstraints
{
    ...
    /// </summary>
    public bool IsLeaf(string signal) => LeafSignals?.Contains(signal) == true;
