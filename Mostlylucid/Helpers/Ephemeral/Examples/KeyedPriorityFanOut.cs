...
public sealed class KeyedPriorityFanOut<TKey, T> : IAsyncDisposable where TKey : notnull
{
        ...
        IReadOnlySet<string>? deferPriorityOn = null)
    {
        var lanes = new[]
        {
            new PriorityLane(
                "priority",
                MaxDepth: maxPriorityDepth,
                CancelOnSignals: cancelPriorityOn,
                DeferOnSignals: deferPriorityOn),
    ...
    public ValueTask DisposeAsync() => _priorityCoordinator.DisposeAsync();
}
