...
";
            ...
            // TODO: Fix OllamaSharp v3.3.0 API compatibility
            // The Chat API has changed in v3.3.0 and requires refactoring
            // For now, this detector is disabled in ServiceCollectionExtensions
            ...
            {
                Model = _options.OllamaModel ?? "qwen2.5:1.5b",
                Messages = new[]
                {
            ...
            var chatResponse = await ollama.Chat(chat, cts.Token);
            var response = chatResponse.Message?.Content ?? string.Empty;
